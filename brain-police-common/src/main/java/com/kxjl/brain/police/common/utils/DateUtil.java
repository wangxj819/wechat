...
public class DateUtil {
	 ...
	 * @Date 2017/6/8 10:42
	 */
	public static String convertWeekDate(String week) {
		int count = -10;
		switch (week){
			case "周1":
				count = 2;
		...
		SimpleDateFormat sdf=new SimpleDateFormat("M月d日"); //设置时间格式
		...
		if(week.indexOf("前天") > -1 || week.indexOf("昨天") > -1 || week.indexOf("今天") > -1 || week
				.indexOf("明天") > -1 || week.indexOf("后天") > -1 ){
			cal.add(Calendar.DATE,count);//把日期往后增加一天.整数往后推,负数往前移动
		}else{
			cal.add(Calendar.DATE,count-day);//根据日历的规则，给当前日期减去星期几与一个星期第一天的差值
		}
		String imptimeBegin = sdf.format(cal.getTime()); //想获取的时间
